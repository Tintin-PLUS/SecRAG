...
完整依赖安装、模型下载、Windows启动和评测步骤统一维护在[子项目README](../README.md)。

Windows建议先手动启动Python Embedding服务（8902），再启动Rust知识库API（8901）。Rust检测到8902服务后会复用它；自动启动分支使用`python3`，不保证所有Windows环境都提供该命令。

本文所有请求中的相对路径均以`embedding-test`为当前工作目录。`test_docs`是评测样例，资料中的公司与数字未在此次整理中做事实核验，不能用作投资依据。
